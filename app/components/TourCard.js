...
const TourCard = ({ card }) => {
  ...
  const primaryImage =
    card.images.find((image) => image.status === "1")?.file ||
    "default-image.jpg";
  ...
  return (
    <div className="bg-white shadow-md rounded-lg overflow-hidden hover:bg-gray-100 hover:shadow-md hover:shadow-yellow-500 transition-all duration-300">
      ...
      {/* Content Section */}
      <div className="p-4">
        <h3 className="text-lg text-black font-bold mb-2">{card.name}</h3>
        <p className="text-gray-600 mb-4 line-clamp-2">{card.detail}</p>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 max-h-[8rem] overflow-hidden">
          {card.highlights.slice(0, maxVisibleItems).map((item, itemIndex) => (
          ...
          {card.highlights.length > maxVisibleItems && (
